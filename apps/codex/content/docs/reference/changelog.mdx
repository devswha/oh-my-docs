...
## Latest release — v0.18.14

Patch release after `0.18.13`: clearer agent/model routing diagnostics, safer goal/planning workflows, sturdier plugin/native hook behavior, cleaner HUD/Team/tmux handling, root-owned artifact detection in doctor, and madmax run-history discovery for resume.

**Changed:** per-agent model overrides, launch diagnostics, completed Codex goal cleanup guidance, Ralplan transition diagnostics, supervised Autopilot rework, Beads metadata handling, and goal/skill workflow guidance.

**Fixed:** plugin AGENTS policy preservation, dev plugin cache diagnostics, bundled skill agent-tier references, null-output native hook success, `PreToolUse` stdout/schema handling, Windows native hook launch, stale Autopilot HUD reporting, tmux paste-buffer cleanup, worker AGENTS guidance, and Ralplan freshness/approval gates.
