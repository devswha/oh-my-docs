...
|---|---|---|
| Claude Code plugin (`oh-my-claudecode@omc`) | In-session skills, agents, hooks, statusline, MCP servers, and slash commands such as `/autopilot`, `/ralph`, `/ultrawork`, and `/team`. | Marketplace plugin install below. |
| Terminal CLI (`omc`, npm package `oh-my-claude-sisyphus`) | Shell commands such as `omc setup`, `omc update`, `omc team`, and `omc ask`. | `npm i -g oh-my-claude-sisyphus@latest` |

Most users want both. The CLI detects the plugin install and avoids double-registering in-session skills.
...
### Optional: Install the Terminal CLI

If you want `omc setup`, `omc update`, `omc team`, and `omc ask` from your shell:

```bash
npm i -g oh-my-claude-sisyphus@latest
```

### Install Without the Plugin (npm-only)

Prefer one command and no plugin at all? This installs OMC's agents, skills, and hooks straight into `~/.claude` and adds the `omc` command:

```bash
npm i -g oh-my-claude-sisyphus@latest && omc setup --no-plugin
```

The `--no-plugin` flag wires the bundled components directly, so you can skip the marketplace steps above.
