...
**New Features**
- Deep-dive: add multi-entity premise audit — Lane 3 enumerates entity dimensions (cohort IDs, tenant IDs, partition keys) before treating zero-row or mismatch results as evidence of a system defect (#2950)
- Deep-dive: add `ownership_scope` to Lane 3 MOVE recommendations — `personal-config`, `shared-config`, `external`, and `project-scoped` are classified; cross-boundary MOVE candidates are flagged and never surfaced as defaults (#2930)
- Deep-dive: pre-flight workflow setup — load `.claude/settings.json` and resolve `omc.deepInterview.ambiguityThreshold` before execution (#2928)
- Skills: new [autoresearch](/docs/skills/workflow/autoresearch) — stateful single-mission improvement loop with strict evaluator contract, decision logs, and `max-runtime` stop behavior

**Bug Fixes (selected)**
- Persistent stop hook + agent output contracts alignment
- Hook startup contract aligned for autopilot and ralplan
- HUD: MiniMax routing independent of credential presence; `minimaxi.com` and `minimax.com` domains supported
- Ask: close stdin for provider spawns to prevent hang in piped environments
- Trace/interview ephemeral artifacts stay under `.omc/state/`; no scratch files in repo root

**Stats:** 25 PRs merged · 3 new features · 9 bug fixes · 13 other changes
