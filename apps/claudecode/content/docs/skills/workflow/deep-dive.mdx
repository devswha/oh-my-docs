...
| ownership_scope | Examples |
|---|---|
| `personal-config` | user-level dotfiles, `~/.claude/`, personal repos, user-only agent rules |
| `shared-config` | company/org repos, team-maintained config, multi-tenant shared rules |
...
Cross-boundary MOVE candidates (for example `personal-config` → `shared-config`) are flagged with an explicit warning and **must not** be surfaced as the default recommendation; same-scope MOVE, COMPRESS, or KEEP is preferred.
