...
<Callout type="info">
The visible page shows the first `100` matches; long lines are truncated at `1024` characters. Use `skip` to page through more. The anchors are consumed by [`edit`](/docs/tools/files).
</Callout>
...
Finalize a queued preview action by applying or discarding it. It is a hidden tool: it only works after another tool (such as [`ast-edit`](/docs/tools/files)) stages a preview and forces a one-shot `resolve` choice.
