...
If no provider is configured, the tool returns `Error: No web search provider configured.` rather than throwing. A preferred provider can be set in settings (the enum covers `auto`, `exa`, `brave`, `jina`, `kimi`, `perplexity`, `anthropic`, `zai`, `tavily`, `kagi`, `synthetic`, `parallel`, `searxng`); it is tried first, then the rest of the chain.
