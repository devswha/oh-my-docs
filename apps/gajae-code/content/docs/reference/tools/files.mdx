...
It shares the [`ast-grep`](/docs/reference/tools/search) pattern grammar: `$NAME` captures a node, `$_` matches one unbound, `$$$NAME` / `$$$` match zero-or-more. Metavariable names must be uppercase and stand for whole nodes; captures from `pat` substitute into `out`.
