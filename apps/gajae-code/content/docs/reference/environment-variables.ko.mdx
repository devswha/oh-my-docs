...
provider 키는 `getEnvApiKey()`로 사용됩니다. provider별 인증 해석: CLI `--api-key` → 저장된 자격증명 → 저장된 OAuth → 환경변수 → `models.yml`의 `apiKey`. 전체 provider 목록은 길지만(70개 이상) 아래는 주요 항목입니다 — 사용하는 provider의 키만 설정하세요.
...
`SEARXNG_BASIC_USERNAME` / `SEARXNG_BASIC_PASSWORD`는 HTTP Basic 인증을 제공합니다. SearXNG는 `~/.gjc/agent/config.yml`의 동등한 `searxng.*` 설정도 읽으며 env 변수는 폴백입니다. Anthropic 웹 검색 인증 체인은 `ANTHROPIC_SEARCH_API_KEY`(+ 선택적 `ANTHROPIC_SEARCH_BASE_URL`)로 시작합니다.
