...
The default dark theme is `red-claw` — the GJC crustacean look, with red/orange brand tokens, separate semantic error/warning/diff-removal tokens, and crab-oriented symbol overrides.
...
1. Built-in embedded themes (`dark.json`, `light.json`, and all bundled `defaults/*.json`, including `red-claw`).
