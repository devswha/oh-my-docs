...
planner produces the plan that drives the [loop](/docs/workflows). During `deep-interview` / `ralplan`, planner sequences the work and defines "done" conditions; [`critic`](/docs/agents/critic) then pressure-tests that plan, and [`executor`](/docs/agents/executor) carries the approved version out under `ultragoal`. The handoff section of the plan says which role to invoke next.
