...
architect is the independent reviewer in the [loop](/docs/workflows). Unlike [`critic`](/docs/agents/critic), which judges a *plan* before execution, architect assesses a *design or change* — typically reviewing what [`executor`](/docs/agents/executor) produced or evaluating an existing codebase, without ever touching files.
