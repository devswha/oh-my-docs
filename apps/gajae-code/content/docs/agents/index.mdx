...
<Cards>
  <Card title="executor" href="/docs/agents/executor">Turns a scoped task into a working, verified change. The only writer.</Card>
  <Card title="architect" href="/docs/agents/architect">Read-only architecture and code review with severity-rated findings.</Card>
  <Card title="planner" href="/docs/agents/planner">Read-only sequencing, acceptance criteria, risks, and handoff shape.</Card>
  <Card title="critic" href="/docs/agents/critic">Read-only plan critique: actionable and verifiable, or not.</Card>
</Cards>

## How they map onto the loop

The role agents map onto the [workflow loop](/docs/workflows):
...
- **architect** provides an independent read-only review — `CLEAR` / `WATCH` / `BLOCK` plus `APPROVE` / `COMMENT` / `REQUEST CHANGES` — when a design or change needs assessment.
