...
`deep-interview` is the first step of the [working loop](/docs/commands). It removes ambiguity before any planning or code changes, so the rest of the loop builds on a clear, agreed understanding rather than a guess.
...
`deep-interview` feeds a clarified problem statement into [`ralplan`](/docs/commands/ralplan), which turns it into a reviewed plan before any mutation. Skipping clarification tends to surface as rework later in [`ultragoal`](/docs/commands/ultragoal), once findings contradict an assumption that was never checked.
