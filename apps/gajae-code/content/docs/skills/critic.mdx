...
critic is the gate before mutation in the [loop](/docs/commands). It pressure-tests the plan that [`planner`](/docs/skills/planner) produced during `ralplan`; only after an OKAY verdict does [`executor`](/docs/skills/executor) start changing files under `ultragoal`. Where [`architect`](/docs/skills/architect) reviews a *design or change*, critic reviews the *plan* — and routes revision needs back to `planner`.
