...
executor is the writer at the end of the [loop](/docs/commands): after [`planner`](/docs/skills/planner) sequences the work and [`critic`](/docs/skills/critic) confirms the plan is actionable, executor carries it out under `ultragoal`. Parallel executor runs can be coordinated with `team`.
