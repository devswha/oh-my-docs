...
| `--platform=opencode\|codex\|both` | Select Ultimate/OpenCode, Light/Codex, or both from the shared installer. |
| `--codex-autonomous` | Configure Codex Light for autonomous agent-style permissions. |
| `--no-codex-autonomous` | Leave Codex permission settings untouched during Light install. |
...
| `OMO_CODEX_SEND_ANONYMOUS_TELEMETRY=0` | Disable Codex Light active-install telemetry. |
| `OMO_CODEX_DISABLE_POSTHOG=1` | Disable Codex Light PostHog telemetry. |
