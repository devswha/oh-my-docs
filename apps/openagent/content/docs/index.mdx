...
If you prefer to run it yourself, choose the edition:
...
Ultimate uses Bun and asks which model subscriptions you have. Light uses Node/npm through `lazycodex-ai`, writes managed Codex plugin state under `~/.codex/`, and does not require Bun.
...
- **Built-in MCPs** — Ultimate uses OpenCode MCP surfaces such as Exa, Context7, and Grep.app; Codex Light adds plugin-scoped `grep_app`, `context7`, `codegraph`, `git_bash`, and `lsp`.
