...
const appPath = argValue('--app', 'apps/openagent');
const appRoot = resolve(root, appPath);
const upstreamUrl = argValue('--upstream', 'https://github.com/code-yeongyu/oh-my-openagent.git');
const branch = argValue('--branch', 'dev');
const cachePath = argValue('--cache', '.omx/cache/upstream/oh-my-openagent');
...
const siteVersion = siteVersionText.match(/OMO_VERSION\s*=\s*["']([^"']+)["']/)?.[1] ?? null;
...
if (json) {
  ...
  console.log('# OmO upstream inspection');
  console.log('');
  ...
  console.log('- When upstream version changes, edit `apps/openagent/src/lib/version.ts` manually (no inject script).');
  console.log('- Run `npm run lint:openagent` and `npm run build:openagent`.');
}
