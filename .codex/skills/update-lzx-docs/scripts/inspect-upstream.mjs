...
const appPath = argValue('--app', 'apps/lzx');
const appRoot = resolve(root, appPath);
const upstreamUrl = argValue('--upstream', 'https://github.com/code-yeongyu/lazycodex.git');
const branch = argValue('--branch', 'main');
const cachePath = argValue('--cache', '.omx/cache/upstream/lazycodex');
...
const siteVersion = siteVersionText.match(/LZX_VERSION\s*=\s*["']([^"']+)["']/)?.[1] ?? null;
...
if (json) {
  ...
  console.log('# LazyCodex upstream inspection');
  console.log('');
  ...
  console.log('- Upstream package.json has a `version` field; the build runs scripts/inject-lzx-version.mjs to refresh `LZX_VERSION` automatically (seed in apps/lzx/src/lib/version.ts).');
  console.log('- Run `npm run lint:lzx` and `npm run build:lzx`.');
}
