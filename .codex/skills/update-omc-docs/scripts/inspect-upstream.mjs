...
const appPath = argValue('--app', 'apps/claudecode');
const appRoot = resolve(root, appPath);
const upstreamUrl = argValue('--upstream', 'https://github.com/Yeachan-Heo/oh-my-claudecode.git');
const branch = argValue('--branch', 'main');
const cachePath = argValue('--cache', '.omx/cache/upstream/oh-my-claudecode');
...
const siteVersion = siteVersionText.match(/OMC_VERSION\s*=\s*["']([^"']+)["']/)?.[1] ?? null;
...
const UPSTREAM_PATHS_OF_INTEREST = [
  'README.md',
  'README.*.md',
  'CHANGELOG.md',
  'package.json',
...
if (diffBase) {
  const diff = runMaybe(
    'git',
    ['diff', '--name-only', `${diffBase}..HEAD`, '--', ...UPSTREAM_PATHS_OF_INTEREST],
    { cwd: upstreamRoot },
  );
  changedFiles = diff ? diff.split('\n').filter(Boolean) : [];
} else {
  const files = runMaybe('git', ['ls-files', ...UPSTREAM_PATHS_OF_INTEREST], { cwd: upstreamRoot });
  changedFiles = files ? files.split('\n').filter(Boolean) : [];
}

const promptFiles = [
  ...walkFiles(join(upstreamRoot, 'prompts')),
  ...walkFiles(join(upstreamRoot, 'agents')),
...
if (json) {
  ...
  console.log('# OMC upstream inspection');
  console.log('');
  ...
  console.log(`- Prompt/agent files: ${promptFiles.length}`);
  ...
  console.log('- When upstream version changes, run `node apps/claudecode/scripts/inject-omc-version.mjs` (or just `npm --prefix apps/claudecode run dev` once) to refresh `src/lib/version.ts`.');
  console.log('- Run `npm run lint:claudecode` and `npm run build:claudecode`.');
}
