...
const appPath = argValue('--app', 'apps/gajae-code');
const appRoot = resolve(root, appPath);
const upstreamUrl = argValue('--upstream', 'https://github.com/Yeachan-Heo/gajae-code.git');
const branch = argValue('--branch', 'main');
const cachePath = argValue('--cache', '.omx/cache/upstream/gajae-code');
...
const siteVersion = siteVersionText.match(/GC_VERSION\s*=\s*["']([^"']+)["']/)?.[1] ?? null;
...
if (json) {
  ...
  console.log('# Gajae-Code upstream inspection');
  console.log('');
  ...
  console.log('- Upstream has no root version field; edit the `GC_VERSION` seed in `apps/gajae-code/src/lib/version.ts` to bump.');
  console.log('- Run `npm run lint:gajae-code` and `npm run build:gajae-code`.');
}
